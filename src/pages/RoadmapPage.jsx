    ...
    <div style={{ borderBottom: "1px solid var(--border)", background: done ? "var(--success-bg)" : "transparent", transition: "background 0.2s" }}>
      ...
      >
        ...
        {topic.concepts_to_master?.length > 0 && (
          <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, letterSpacing: "0.1em", color: "var(--text-dim)", padding: "2px 8px", border: "1px solid var(--border)", borderRadius: 2 }}>{topic.concepts_to_master.length} CONCEPTS</span>
        )}
        {hasContent && (
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{ color: "var(--accent-border)", transition: "transform 0.2s", transform: open ? "rotate(180deg)" : "rotate(0)", flexShrink: 0 }}>
        ...
        {open && hasContent && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }}
            ...
            <div style={{ padding: "16px 24px 20px 80px", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 16 }}>
              {topic.concepts_to_master?.length > 0 && (
                <div>
                  <div style={{ fontSize: 9, letterSpacing: "0.16em", color: "var(--accent)", marginBottom: 10 }}>CONCEPTS</div>
                  {topic.concepts_to_master.map((c, i) => (
                    <div key={i} style={{ display: "flex", gap: 8, fontSize: 11, color: "var(--text-muted)", lineHeight: 1.8 }}>
                      <span style={{ color: "var(--accent-border)", flexShrink: 0 }}>▸</span>{String(c)}
                    </div>
                  ))}
                </div>
              )}
              {topic.subtopics?.length > 0 && (
                <div>
                  <div style={{ fontSize: 9, letterSpacing: "0.16em", color: "var(--info)", marginBottom: 10 }}>SUBTOPICS</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {topic.subtopics.map((s, i) => <span key={i} style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, padding: "3px 8px", border: "1px solid var(--border)", borderRadius: 2, color: "var(--info)" }}>{String(s)}</span>)}
                  </div>
                </div>
              )}
              {topic.recommended_practice?.length > 0 && (
                <div>
                  <div style={{ fontSize: 9, letterSpacing: "0.16em", color: "var(--success)", marginBottom: 10 }}>PRACTICE</div>
                  {topic.recommended_practice.map((p, i) => (
                    <div key={i} style={{ display: "flex", gap: 8, fontSize: 11, color: "var(--text-muted)", lineHeight: 1.8 }}>
                      <span style={{ color: "var(--success-border)", flexShrink: 0 }}>▸</span>{String(p)}
                    </div>
                  ))}
                </div>
              )}
              {topic.mini_project && (
                <div style={{ gridColumn: "1/-1", padding: "12px 14px", border: "1px solid var(--accent-border)", borderRadius: 2, background: "var(--accent-dim)" }}>
                  <div style={{ fontSize: 9, letterSpacing: "0.16em", color: "var(--accent-bright)", marginBottom: 6 }}>⚡ MINI PROJECT</div>
                  <div style={{ fontSize: 12, color: "var(--accent)" }}>{typeof topic.mini_project === "string" ? topic.mini_project : topic.mini_project.title}</div>
                </div>
              )}
            </div>
          </motion.div>
    ...
    <div style={{ border: `1px solid ${allDone ? "var(--success-border)" : "var(--border)"}`, borderRadius: 4, overflow: "hidden", marginBottom: 8, transition: "border-color 0.3s" }}>
      ...
      >
        ...
        <div style={{ width: 40, height: 40, border: `1px solid ${allDone ? "var(--success-border)" : "var(--border)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "border-color 0.3s" }}>
          {allDone
            ? <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M3 9L7.5 13.5L15 5" stroke="var(--success)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
            : <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, color }}>{String(index + 1).padStart(2, "0")}</span>
          }
        </div>

        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, letterSpacing: "0.16em", color: allDone ? "var(--success)" : "var(--text-dim)", marginBottom: 4, opacity: 0.7 }}>
            PHASE {index + 1} · {doneCount}/{topicKeys.length} TOPICS
          </div>
          <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 20, letterSpacing: "0.04em", color: allDone ? "var(--success)" : "var(--text-heading)", transition: "color 0.3s" }}>
            {(phase.phase_title || `PHASE ${index + 1}`).toUpperCase()}
          </div>
          {topicKeys.length > 0 && (
            <div style={{ marginTop: 8, height: 2, background: "var(--border)", borderRadius: 1, width: 160, overflow: "hidden" }}>
              <div style={{ height: "100%", background: allDone ? "var(--success)" : color, borderRadius: 1, width: `${phasePct}%`, transition: "width 0.4s ease, background 0.3s" }} />
            </div>
          )}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
          {phase.duration && (
            <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "var(--text-muted)", padding: "4px 10px", border: "1px solid var(--border)", borderRadius: 2 }}>{phase.duration}</span>
          )}
          <button
            onClick={togglePhase}
            ...
            style={{
              padding: "5px 10px",
              background: allDone ? "var(--success-bg)" : "var(--input-bg)",
          ...
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ color: "var(--text-dim)", transition: "transform 0.25s", transform: open ? "rotate(180deg)" : "rotate(0)" }}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} transition={{ duration: 0.3, ease: [0.4,0,0.2,1] }} style={{ overflow: "hidden" }}>
            {phase.outcome && (
              <div style={{ padding: "14px 24px", background: "var(--success-bg)", borderTop: "1px solid var(--success-border)", borderBottom: "1px solid var(--border)", display: "flex", gap: 12, alignItems: "flex-start" }}>
                <span style={{ fontSize: 10, color: "var(--success)", flexShrink: 0, marginTop: 1 }}>✓</span>
                <div>
                  <div style={{ fontSize: 9, letterSpacing: "0.14em", color: "var(--success)", marginBottom: 4, opacity: 0.6 }}>PHASE OUTCOME</div>
                  <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: "var(--success)", lineHeight: 1.6, opacity: 0.85 }}>{phase.outcome}</div>
                </div>
              </div>
            )}
    ...
    <>
      <style>{`
        ...
        .rp-root {
          ...
          padding:calc(var(--navbar-height,56px)) 0 80px;
          color: var(--text);
        ...
        .rp-sticky {
          position:sticky; top:var(--navbar-height,56px); z-index:100;
        ...
        .rp-stat-row { display:flex; gap:0; border-top:1px solid var(--border); margin-top:20px; }
        .rp-stat-cell { padding:12px 20px; border-right:1px solid var(--border); flex:1; }
        .rp-stat-cell:last-child { border-right:none; }
        .rp-stat-label { font-size:8px; letter-spacing:0.18em; color: var(--text-dim); margin-bottom:4px; text-transform:uppercase; }
        .rp-stat-value { font-family:'Bebas Neue',sans-serif; font-size:20px; color: var(--accent); letter-spacing:0.04em; }
        .rp-tabs { display:flex; border-top:1px solid var(--border); }
        .rp-tab {
          padding:13px 22px; font-size:10px; letter-spacing:0.14em; color: var(--text-dim);
          cursor:pointer; background:none; border:none; font-family:'IBM Plex Mono',monospace;
        ...
        .back-btn {
          ...
          font-size:10px; letter-spacing:0.1em; cursor:pointer; border-radius:2px;
        ...
        .week-num {
          width:36px; height:36px; border:1px solid var(--accent-border); background: var(--accent-dim);
          display:flex; align-items:center; justify-content:center;
          font-family:'Bebas Neue',sans-serif; font-size:16px; color: var(--accent); z-index:1;
        }
      ...
      <div className="rp-root">
                  ...
                  <button key={t} className={`rp-tab${tab===t?" active":""}`} onClick={() => setTab(t)}>
                    ...
                    {total > 0 && (
                      <span style={{
                        ...
                        background: done===total ? "var(--success-bg)" : "var(--accent-dim)",
                        color: done===total ? "var(--success)" : "var(--accent)",
                        fontSize:9, fontFamily:"'IBM Plex Mono',monospace",
                        border: `1px solid ${done===total ? "var(--success-border)" : "var(--accent-border)"}`,
                      }}>
                        {done}/{total}
                      </span>
                    )}
        ...
        <div className="rp-content">
          ...
          {error && (
            <div style={{ padding:"16px 20px", background:"var(--danger-bg)", border:"1px solid var(--danger-border)", borderRadius:2, color:"var(--danger)", fontSize:12, marginBottom:32 }}>
              ✕ {error}
            </div>
          )}
                      ...
                      <div key={i} className="week-card" style={{ opacity: done ? 0.7 : 1, transition:"opacity 0.2s" }}>
                        ...
                        <div className="week-body">
                          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12, marginBottom:14 }}>
                            <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:13, fontWeight:600, color: done ? "var(--success)" : "var(--text-heading)", textDecoration: done ? "line-through" : "none", transition:"all 0.2s" }}>
                              {w.focus || `Week ${w.week}`}
                            </div>
                          ...
                          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:12 }}>
                            {w.topics_to_cover?.length > 0 && (
                              <div>
                                <div style={{ fontSize:8, letterSpacing:"0.18em", color:"var(--accent)", marginBottom:8, opacity:0.6 }}>TOPICS</div>
                                {w.topics_to_cover.map((t,j) => {
                                  const tk = `week-${i}-topic-${j}`;
                                  const td = !!progress[tk];
                                  return (
                                    <div key={j} style={{ display:"flex", gap:8, fontSize:11, color: td ? "var(--success)" : "var(--text-muted)", lineHeight:1.8, alignItems:"center", textDecoration: td ? "line-through" : "none" }}>
                            ...
                            {w.practice_goals?.length > 0 && (
                              <div>
                                <div style={{ fontSize:8, letterSpacing:"0.18em", color:"var(--info)", marginBottom:8, opacity:0.6 }}>PRACTICE</div>
                                {w.practice_goals.map((g,j) => {
                                  const pk = `week-${i}-practice-${j}`;
                                  const pd = !!progress[pk];
                                  return (
                                    <div key={j} style={{ display:"flex", gap:8, fontSize:11, color: pd ? "var(--success)" : "var(--text-muted)", lineHeight:1.8, alignItems:"center", textDecoration: pd ? "line-through" : "none" }}>
                          ...
                          {w.project_milestone && (
                            <div style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 12px", background:"var(--success-bg)", border:"1px solid var(--success-border)", borderRadius:2, fontSize:11, color:"var(--success)" }}>
                          ...
                          <div style={{ padding:20 }}>
                            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                              <span style={{
                                ...
                                padding:"3px 8px",
                                border:`1px solid ${lvl==="beginner" ? "var(--success-border)" : lvl==="intermediate" ? "var(--accent-border)" : "var(--danger-border)"}`,
                                borderRadius:2,
                                color: lvl==="beginner" ? "var(--success)" : lvl==="intermediate" ? "var(--accent)" : "var(--danger)",
                            ...
                            <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:20, letterSpacing:"0.04em", color: done ? "var(--success)" : "var(--text-heading)", marginBottom:8, textDecoration: done ? "line-through" : "none", transition:"all 0.2s" }}>
                            ...
                            {p.features?.length > 0 && (
                              <div style={{ marginBottom:12 }}>
                                <div style={{ fontSize:8, letterSpacing:"0.16em", color:"var(--info)", marginBottom:8, opacity:0.6 }}>FEATURES</div>
                                <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                                  {p.features.map((f,fi) => <span key={fi} style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, padding:"3px 8px", border:"1px solid var(--border)", borderRadius:2, color:"var(--info)" }}>{f}</span>)}
                                </div>
                              </div>
                            )}
                            {p.core_topics_used?.length > 0 && (
                              <div>
                                <div style={{ fontSize:8, letterSpacing:"0.16em", color:"var(--accent)", marginBottom:8, opacity:0.6 }}>TECH STACK</div>
                                <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                                  {p.core_topics_used.map((t,ti) => <span key={ti} style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, padding:"3px 8px", border:"1px solid var(--accent-border)", borderRadius:2, color:"var(--accent)" }}>{t}</span>)}
                                </div>
                              </div>
                            )}
                          </div>
                    ...
                    return (
                      <div key={i} className="iv-card" style={{ border: done ? "1px solid var(--success-border)" : undefined, opacity: done ? 0.8 : 1, transition:"all 0.2s" }}>
                        <div style={{ borderBottom:"1px solid var(--border)", padding:"18px 20px" }}>
                          ...
                          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:20, color: done ? "var(--success)" : "var(--text-heading)", textDecoration: done ? "line-through" : "none", transition:"all 0.2s" }}>
                            {(s.stage||`STAGE ${i+1}`).toUpperCase()}
                          </div>
                          {s.focus_areas?.length > 0 && (
                            <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginTop:12 }}>
                              {s.focus_areas.map((a,ai) => <span key={ai} style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, letterSpacing:"0.08em", padding:"3px 8px", border:"1px solid var(--accent-border)", borderRadius:2, color:"var(--accent)" }}>{String(a)}</span>)}
                            </div>
                          )}
                        </div>
                        <div style={{ padding:"16px 20px" }}>
                          {s.common_questions?.length > 0 && (
                            <div style={{ marginBottom:16 }}>
                              <div style={{ fontSize:8, letterSpacing:"0.16em", color:"var(--text-dim)", marginBottom:10 }}>COMMON QUESTIONS</div>
                          ...
                          {s.mock_strategy && (
                            <div style={{ padding:"10px 12px", background:"var(--accent-dim)", border:"1px solid var(--accent-border)", borderRadius:2, fontSize:11, color:"var(--accent)", lineHeight:1.6 }}>
                              💡 {s.mock_strategy}
                            </div>
                          )}
                        </div>
